...
export class FirebaseDmsFeaturesNoonEnabledService extends AbstractService {
  private static _instance: FirebaseDmsFeaturesNoonEnabledService;

  public static getInstance(): FirebaseDmsFeaturesNoonEnabledService {
    if (_.isNil(FirebaseDmsFeaturesNoonEnabledService._instance)) {
      FirebaseDmsFeaturesNoonEnabledService._instance = new FirebaseDmsFeaturesNoonEnabledService();
    }

    return FirebaseDmsFeaturesNoonEnabledService._instance;
  }

  public constructor() {
    super(ServiceNameEnum.FIREBASE_DMS_FEATURES_NOON_ENABLED_SERVICE);
  }
   ...
   * @param   {boolean}           isEnabled  The new [enabled state]{@link IFirebaseDmVFinal#features#noon#isEnabled}.
  ...
  private _isDmFullyUpToDate(firebaseDm: IFirebaseDmVFinal): boolean {
    return this._isValidFeature(firebaseDm) && this._isValidNoonFeature(firebaseDm);
  }
  ...
  private _isValidNoonFeature(firebaseDm: IFirebaseDmVFinal): boolean {
    return FirebaseDmsFeaturesNoonService.getInstance().isValid(firebaseDm.features?.noon);
  }
  ...
  private _getUpdatedDm(isEnabled: boolean, firebaseDm: IFirebaseDmVFinal): IObject {
    ...
    firebaseDm.features.noon = FirebaseDmsFeaturesNoonService.getInstance().getUpToDate(firebaseDm.features.noon);
    firebaseDm.features.noon.isEnabled = isEnabled;
  ...
  /**
   * @description
   * Update the state of the noon feature to either disable or enable it.
  ...
  private _getUpdatedDmWithPathOnly(isEnabled: boolean): IObject {
    ...
    _.set(flattenFirebaseDm, `features.noon.isEnabled`, isEnabled);
  ...
  private _logUpdatingStateStart(): void {
    LoggerService.getInstance().debug({
      context: this._serviceName,
      message: ChalkService.getInstance().text(`updating Firebase DM noon feature enabled state...`),
    });
  }

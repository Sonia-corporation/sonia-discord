...
FS.createFile(`${APP_ROOT_PATH.path}/dist/firebase-service-account-file.json`)
  .then(() => {
    LOGGER.success(CONTEXT, CHALK.text(`Firebase service account file successfully created into dist`));
    LOGGER.debug(CONTEXT, CHALK.text(`Rewrite Firebase service account file content into dist...`));

    FS.writeJson(`${APP_ROOT_PATH.path}/dist/firebase-service-account-file.json`, {
      ...
      .then(() => {
        LOGGER.success(CONTEXT, CHALK.text(`Firebase service account file content successfully rewritten into dist`));
      })
      .catch((error) => {
        LOGGER.error(CONTEXT, CHALK.text(`Failed to rewrite Firebase service account file content into dist`));
        LOGGER.error(CONTEXT, CHALK.text(error));
      });
  })
  .catch((error) => {
    LOGGER.error(CONTEXT, CHALK.text(`Failed to create Firebase service account file into dist`));
    LOGGER.error(CONTEXT, CHALK.text(error));
  });

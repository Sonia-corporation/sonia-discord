...
FS.copy(`${APP_ROOT_PATH.path}/src/environment/secret-environment.json`, `${APP_ROOT_PATH.path}/dist/environment.json`).then(() => {
  ...
  LOGGER.debug(CONTEXT, CHALK.text(`On the CI, this is not a problem since the file should be already set up on the server`));
  LOGGER.debug(CONTEXT, CHALK.text(`On local development, it means that you HAVE to create the file with at least a "discord.sonia.secretToken" inside it`));
});
